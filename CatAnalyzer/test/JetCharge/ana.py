...
def anaTree( tree, label="true" ) :
  ...
  for idx, pdgid in enumerate([5,-5]) :
    ...
    h2[idx].Sumw2()
    if ( h2[idx].GetEntries() ==0) : continue
  ...
  h3 = []
  for idx, pdgid in enumerate([5,-5]) : 
    h3.append( getTH1( "delta_jetq_pdgid_%d"%(pdgid),[10,-5,5],tree,"(jet_charge[0]-jet_charge[1])","%s&&jet_pdgId[0]==%d"%(default_cut,pdgid)))
    h3[idx].Sumw2()
    if ( h3[idx].GetEntries() ==0 ) : continue 
    fr = h3[idx].Fit('gaus',"S")
    separation.append(fr.GetParams()[1])
